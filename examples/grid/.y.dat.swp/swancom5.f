...
!               this subroutine is not activated in SWAN since it is
!               based on a nonstationary model (limiting with dt) )
...
!     belong to the first (or other) sweep (e.g. subroutine SOLPRE).
!
...
!     XCGRID: Coordinates of computational grid in x-direction            30.82
!     YCGRID: Coordinates of computational grid in y-direction            30.82
!
...
!     INTEGERS:
!     --------------------------------------------------------------
!     IXCGRD(1)                Counter of gridpoints in x-direction
!     IYCGRD(1)                Counter of gridpoints in y-direction
...
!  6. Local variables
!
!     VIRT  : To define virtual point for 1D mode                         32.02
!
      REAL    VIRT                                                        32.02
!
!  8. Subroutines used
...
!     Changed ICMAX to MICMAX, since MICMAX doesn't vary over gridpoint   40.22
     ...
     &          RDX(10), RDY(10)    ,                                     40.08
     &          DET,DET2,                                                 40.08
     &          DX1,DX2,DX3,DX4,                                          40.08
     &          DY1,DY2,DY3,DY4                                           40.08
!
...
!     40.14, Nov. 00: Land points exluded (bug fix)
...
!
!     SWP_ARRAY: Array containint the order of sweep handling
!
...
!     *** for IREFR is set 0 (CAD = 0) or ITFRE = 0 (CAS = 0 ) we ***
!     *** have put the IF statement outside the internal loop     ***
!     *** above                                                   ***
!
...
!             *** for first point an upwind scheme is used ***
              ...
              FLEFT = 0.
            ELSE IF ( IS .EQ. MSC ) THEN
!             *** for the last discrete pointin frequency space ***
...
!  2. Purpose
!
!     comp. of @[CAS AC2]/@S initial & boundary : TOLMAN METHOD
!
!  3. Method
!
!     Compute the derivative in s-direction:
...
!                   DS                                DS
!
!     FLR = F(IS+0.5) = 0.5 ( CAS(IS)*AC2(IS) + CAS(IS+1)*AC2(IS+1) )
!     FLL = F(IS-0.5) = 0.5 ( CAS(IS)*AC2(IS) + CAS(IS-1)*AC2(IS-1) )
!
!     IF FLR is < 0 then -FRL
!               > 0 then +FRL
!
!        limit the flux:
!        FLUXMAX = MIN ( AC2 * DS / DT , FLRR )
!
!        FLL is > 0 then +FLL
!               < 0 then -FLL
!
!        limit the flux:
!        FLUXMAX = MIN ( AC2 * DS / DT , FLLL )
!
!  4. Argument variables
!
!     SPCSIG: Relative frequencies in computational domain in sigma-space 30.72
...
!        IDCMIN  1D  frequency dependent counter
!        SECTOR  1D  Counter for number enclosed sectors
!
!     5. SUBROUTINES CALLING
!
!        ACTION
      ...
      DO 200 IS = 1, MSC
        IF ( SECTOR(IS) .GT. 0 ) THEN
          DO 100 IDDUM = IDCMIN(IS), IDCMAX(IS)
            ID = MOD ( IDDUM - 1 + MDC , MDC ) + 1
            IF ( IS .EQ. 1) THEN
!             *** for the first point an upwind scheme is used ***
!             *** the term F(ID,IS-0.5) is equal zero assuming ***
!             *** that the action is zero at a node left from  ***
!             *** IS = 1                                       ***
!
              DS = SPCSIG(IS+1) - SPCSIG(IS)                              30.72
...
!             *** for the last point an upwind scheme is used  ***
!             *** the term F(ID,IS+0.5) is equal zero          ***
!             *** THIS PART HAS TO BE MODIFIED ??????????????  ***
!             *** IN CASE THE CUTOFF FREQUENCY IS NOT EQUAL TO ***
!             *** MSC                                          ***
!
              DS = SPCSIG(IS) - SPCSIG(IS-1)                              30.72
...
!
              DS = 0.5 * ( SPCSIG(IS+1) - SPCSIG(IS-1) )                  30.72
!
!             *** average propagatuion velocity and flux in the ***
!             *** two cross sections adjacent the node IS       ***
!
              FLUXL  = 0.5 * ( CAS(ID,IS,1)    * AC2(ID,IS,KCGRD(1)) +
     &                         CAS(ID,IS-1,1)  * AC2(ID,IS-1,KCGRD(1)) )
              FLUXR  = 0.5 * ( CAS(ID,IS,1)    * AC2(ID,IS,KCGRD(1)) +
...
!     2. PURPOSE
!
!        to predict the action density depending of the sweep
!        direction. A good prediction is necessary for a first
!        accurate prediction of the action density to compute
!        the dissipation of energy. To compute the energy dissipation
!        a prediction is needed at time T.
!
!     3. METHOD
!
!
...
!
!     *** When a current is present the propagation direction of the ***
!     *** energy is not equal to the direction normal to the crest   ***
!     *** of the waves. The dependent variables are CAX and CAY.     ***
!     *** Because the current is already discount in these variables ***
!     *** in the subroutine SPROSD, the subroutine SPREDT is valid   ***
!     *** for the case with or without currents !                    ***
!
!
!     *** for the four sweeps  ***           10/FEB
!
        DO 200 IS = 1, ISSTOP
...
!  2. Purpose
!
!     computes the wave parameters K, CGO, CG in the nearby
...
!  3. Method
!
!     Compute wave number K, group velocity CGO by linear
!     interpolation from 1-dimensional tables.
!     The wave number K(IS,iC) is computed with the dispersion relation:
!
!     S = GRAV K(IS,IC)tanh(K(IS,IC)DEP(IX,IY))
!
!     where S = is logaritmic distributed via LOGSIG
!
...
!     DIR = ID * DD
!
!     In absense of a current the group velocity CG is equal to
!     the group velocity CGO
!
!     CG(ID,IS,IC) = CGO(IS,IC)
!
!  4. Argument variables
!
!     SPCSIG: Relative frequencies in computational domain in sigma-space 30.72
...
!        MSC         Maximum counter of relative frequency
!        MDC         Maximum counter of directional distribution
!        NSMIN       Minimum number of values in the arrays KTAB1D and
!                    CGTAB1D
!        NSMAX       Maximum number of values in the arrays KTAB1D and
!                    CGTAB1D
!
!        REALS:
!        ---------
!
!        CGOND       Non dimensional wave number in absence of
!                    currents
!        DINV        Equal to 1/depth
!        DSND        Non dimensional width of frequency band
!        FAC         Dummy variable for the linear interpolation
!        GRAV        Gravitational acceleration
!        KND         Non dimensional wave number
!        ROOTDG      Depth devided by the gravitational accerelation
!        RPDS        Reciproce value of the variable  DSND
!        SFAC        Dummy variable for the linear interpolation
!        SGD         Group velocity at extremely shallow water ugd
!
!        one and more dimensional arrays:
!        ---------------------------------
!
...
!                        direction of wave propagation in absence of currents
!        CG        3D    Group velocity as function of X, Y and S and D in the
!                        direction of wave propagation in presence of currents
!        DEP1      2D    Depth as function of X and Y at time T
!        ECOS      1D    Represent the values of cos(d) of each spectral
!                        direction
!        ESIN      1D    Represent the values of sin(d) of each spectral
!                        direction
...
!        UY2       2D    Y-component of current velocity of X and Y at
!                        time T+1
!        KTAB1D    2D    Table with the values to compute the non
!                        dimensional wavenumber by linear interpolation
!        CGTB1D    2D    Table with the values to compute the non
!                        dimensional group velocity in absence of
!                        currents by linear interpolation
!
!     5. SUBROUTINES CALLING
!
!        SWOMPU
...
!       CG(ID,IS,IC)=  0.    ,  {Cgo in dir. of wave prop. with currents}
...
!           Compute group velocity in direction of wave propagation
!           with the current component in the direction of the wave
!           propagation
!         else
!           group velocity CG is equal to group velocity
!           without currents CGO
!         end if
!     ----------------------------------------------------------
!   end if
!   ------------------------------------------------------------
...
!       *** call kscip1 to compute KWAVE, and CGO ***
 ...
 70       CONTINUE
!
!         *** compute the group velocity in presence of current
!
          IF (ICUR .GE. 1) THEN
            DO 180 IS = 1, MSC
              DO 170 ID = 1, MDC
