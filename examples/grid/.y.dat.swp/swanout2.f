...
!                                                                             30.81
      ...
      INTEGER IF, IL                                                      40.30
      INTEGER, SAVE :: IREC=0
      LOGICAL, SAVE :: MATLAB=.FALSE.                                     40.30
...
!       output to datafile without header
  ...
  21  FORMAT (' Test SWBLOK: RTYPE NREF NVAR ',A4,2(1X,I6))
      IF (NREF.EQ.0) THEN
        ...
        MATLAB = INDEX( FILENM, '.MAT' ).NE.0 .OR.                        40.30
     &           INDEX (FILENM, '.mat' ).NE.0                             40.30
...
!
      ...
      DO JVAR = 1, NVAR
         ...
         IF ( OVSVTY(IVTYPE).LT.3 ) THEN
            DO IYK = 1, MYK
               IP = (IYK-1)*MXK
               WRITE (NREF, FLT_BLKP)
     &                             (VOQ(IP+IXK,VOQR(IVTYPE)), IXK=1,MXK)
            END DO
         ELSE
            DO IYK = 1, MYK
               IP = (IYK-1)*MXK
     ...
     &                             (VOQ(IP+IXK,VOQR(IVTYPE)), IXK=1,MXK)
            ...
            DO IYK = 1, MYK
               IP = (IYK-1)*MXK
               WRITE (NREF, FLT_BLKP)
     &                           (VOQ(IP+IXK,VOQR(IVTYPE)+1), IXK=1,MXK)
            END DO
         END IF
      END DO
...
!
 ...
 220      FORMAT (A6)
          GOTO 290
        ENDIF
        IF ( PARLL .AND. IONOD(IP).NE.INODE ) THEN                        40.31
          WRITE (NREF, 220) 'NODATA'                                      40.31
          GOTO 290                                                        40.31
        END IF                                                            40.31
        IF (ICUR.GT.0) THEN
          UX = VOQ(IP,VOQR(5))
...
!
          ELSE
!           exception values for DIR and DSPR
...
!     --- if necessary, give message that error occurred while writing file
      ...
      IF ( IOS.NE.0 ) THEN
         ...
         MSGSTR = 'Error while writing binary MAT-file'//NWL//
