...
!
!     b. For SWIND3 (wind input accodring to Snyder (1981) ***
!
...
!C
!C       *** wind input term according to Janssen (1991) ***
!C
!C        IF ( WIND10 .GT. 7.5 ) THEN
!C          UFRIC = WIND10 * SQRT (( 0.8 + 0.065 * WIND10 ) * 0.001 )
!C        ELSE
!C          CDRAG = 0.0012873
!C          UFRIC = SQRT ( CDRAG ) * WIND10
!C        ENDIF
!C
!C        UFRIC = MAX ( 1.E-15 , UFRIC )
!C        FPM =  GRAV / ( 28.0 * UFRIC )
!C
!C      END IF
!
!     *** test output ***
...
!
!         This input term should be comninated with the dissipation
...
!
!/TC           *** an fully explicit computation is favourable ***
!/TC           *** when integrating the wind input source term ***
!/TC
!/T            IMP_EXP = .FALSE.
!/T            IF ( IMP_EXP) THEN
!/T              IMATDA(ID,IS) = IMATDA(ID,IS) - SWINEB
!/T              IMATRA(ID,IS) = IMATRA(ID,IS) + SWINEA
!/T            ELSE
!/T
...
!
!        *** nonstationary and no first guess and only and only      ***
!        *** for the first time step (the number of iterations       ***
!        *** however still can increase per time step (ITERSTA= 2,3) **
!
...
!        *** stationary and no first guess and only and only  ***
!        *** for the first iteration ime step                 ***
!        *** This corresponds to ordinairy calculation without***
!        *** any first guess possibilities                    ***
!
        ZO     = ALPHA * UFRIC * UFRIC / GRAV
        ZE     = ZO / SQRT( 1. - RATIO )
        USTAR(KCGRD(1)) = UFRIC                                           30.21
...
!
!
!        *** stationary and a first guess (this subroutine is never ***
...
!     reduces to teh Snyder 91982) expression form for spectral
!     wave components with weak wind forcing and to the Plant (1982)
!     form for more strongly froced wave components:
!
!  3. Method
!
