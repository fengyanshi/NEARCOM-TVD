...
!     SWCAP   (White capping: six formulations )                          40.30
...
!     ----------------------
!
!     The bottom dissipation aplying Madsen formulation is as
!     follows:
!
!                          fw [n - 1/2] UBR E(s,d)
!     [1]    Sdsb(s,d) = -  ------------------------
...
!                        2 g sinh (kD)
!
!     UBOT(IX,IY) is computed in the subroutine SDISPA. The friction
...
!     IBOT        Indicator if bottomfriction is on
...
!     KD          Wavenumber * Depth
!     SBOTEO      Sourceterm for the bottomfriction to be stored
!                 in the array IMATDA
...
!     PBOT      1D    Coefficient for bottomfriction models
...
!     DISSC1    2D    Dissipation coefficient, fun of sigma and theta
...
!                         HM
!
!        The following appproximation is applied (Dingemans 1983):
!
!                       2  1/2
!        b = ( 8 ETOT/HM  )
!
...
!     |   Qb = Qo                                 0.9 < b < 1.0
!
!        The parameters ETOT and HM are determined in the subroutine
!        SDISPA
!
!     4. PARAMETERLIST
!
...
!        ---------
!        B     Dummy variable
!        ETOT  Total energy per spatioal gridpoint
!        QO    First estiamte of the fraction of breaking waves
...
!       Compute factor b according to equation (3)
!     Else
!       b = 0
!     ------------------------------------------------------------
!     Compute first estimate Qo according to equation (4)
!     Compute Qb according to equation (5)
!   ------------------------------------------------------------
!   End of FRABRE
!   ------------------------------------------------------------
!
...
!  1. Updates
!
!     30.77, Sep 1998: FRABRE2 is an update of FRABRE: the discontinuity at
!                      B = 0.9 in FRABRE has been removed and the discontinuity
!                      at B = 0.3 is changed in a discontinuity at B = 0.2
!                      for which QBLOC = 1.E-9
!
!  2. Purpose
!
!     to compute the fraction of breaking waves in point ix,iy
...
!
!
!     The local maximum wave height Hm is computed in subroutine SSDIPA.
!     The fraction of breaking waves Qb is calculated in the subroutine FRABRE2
!
!     The new value for the dissipation is computed implicitely using
...
!             d N       d N
!
!     Since BB and SMEBRK * N are proportional
!
...
!     DIS0    Dummy variable
!     FRDEP   Frequency dependent function
!     ID      Counter for directional steps
!     IDDUM   Counter
!     IENT    Number of entries
!     IS      Counter for frequency steps
!     NORM    Normalization factor for frequency dependent function
!     RES     Dummy variable
!     RESD    Dummy variable
...
!
!     ------------------------------------------------------------
!     Get HM, QB and ETOT from the subroutine SDISPA
...
!     40.32, May. 04: Normalisation added to CSM
!
!
!  2. Purpose
!
...
!
!                                                                         40.32
!     Recent modifications are described in:                              40.32
!     Hurdle, D.P., and G.Ph. van Vledder, 2004: Improved spectral wave   40.32
!     modelling of white-capping dissipation in swell sea systems. Proc.  40.32
!     Offshore Mechanics, Arctic Engineering Conference, June 2004,       40.32
!     Vancouver, Canada,  paper OMAE2004-51562.                           40.32
!                                                                         40.32
!     Default values: C_st = 4                                            40.32
!                     m    = 2                                            40.32
!                                                                         40.32
!     C_CSM = A*C_st S(sig,th)                                            40.32
!
!     where
!
...
!
!                                                                         40.32
!     A is a normalisation coefficient computed                           40.32
!                                                                         40.32
!              1     GAMMA(0.5*m+1.)                                      40.32
!       A = -------- ----------------                                     40.32
!           SQRT(PI) GAMMA(0.5*m+0.5)                                     40.32
!
...
!     ISSTOP: Maxmum counter in freuqency space that is propagated within a sweep ??
!     KWAVE : Wavenumber
...
!
      IF (IWCAP.GT.6) THEN                                                40.30
!
! Error message
!
        CALL MSGERR(4,'Value for IWCAP should be less than 7!')           40.30
        RETURN
      END IF
!
! Initialisation
...
!                                                                         40.31
         ...
         END DO                                                           40.31
!
!        --- compute normalisation coefficient                            40.32
!
         CPOW = PWCAP(13)                                                 40.32
         IF (CPOW > 10) THEN                                              40.32
           CTOT = SQRT(CPOW/(2.*PI))*(1.+0.25/CPOW)                       40.32
         ELSE                                                             40.32
           CTOT = 1./SQRT(PI)*GAMMA(0.5*CPOW+1.)/GAMMA(0.5*CPOW+0.5)      40.32
         END IF                                                           40.32
!
         CUMSTP = 0.                                                      40.30
         DO IS = 1,ISSTOP                                                 40.30
            BINSIZE = SPCSIG(IS)*DELTA*DSIGMA(IS)                         40.32
            DO ID1 = 1,MDC                                                40.30
               CUMSTP(IS,ID1) = CUMSTP(IS-1,ID1)                          40.31
               DO ID2 = 1,MDC                                             40.30
                  EBIN= AC2(ID2,IS,KCGRD(1))*BINSIZE                      40.32 40.30
...
!               *** nonlinear interactions ***
!
                TRIA = TRIA + PLTRI(ID,IS,IPTST)*SPCSIG(IS)                          40.31 40.00
!
                NL4S = NL4S + PLNL4S(ID,IS,IPTST)*SPCSIG(IS)                         40.31 40.00
!
                IF ( IQUAD .EQ. 1) THEN
...
!
     ...
     &                              + PLWNDB(ID,IS,IPTST) * SIGACT
                ...
                PLNL4S(ID,IS,IPTST) = PLNL4S(ID,IS,IPTST) * SPCSIG(IS)                 40.31
