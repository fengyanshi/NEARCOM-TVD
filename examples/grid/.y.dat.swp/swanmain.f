...
!     SWINCM: Initialize common blocks
!     SWINIT: Initialize several variables and arrays
!     SWPREP: Computation of the transformation coefficients between the
!             different grids
...
! 13. Source Code
!
!     --- initialize common blocks                                        40.31

! skip initialization after the first master call fyshi

      if(Master_Start.eq.1) then
      CALL SWINCM                                                         40.31

!     --- initialize the MPI execution environment                        40.30
!         and common block SWPARALL                                       40.30
...
!     --- start SWAN run
      ...
      print*, 'start swmain, num of calls',istep+1
...
!     IONOD :     array indicating in which subdomain and at what time    40.30
!                 output points are located                               40.30
!     IONODC:     auxiliary array for collecting the array IONOD          40.30
!     IUNIT :     counter for file unit numbers                           34.01
!     LOPEN :     indicates whether a file is open                        34.01
!
      INTEGER   IUNIT, INDX                                                     34.01
...
!
      INTEGER, ALLOCATABLE :: IONOD(:), IONODC(:)                         40.30
!
!  8. Subroutines used
!
!     SWINIT
...
! 13. Source text
!
! skip initialization after first call -fyshi
...
! remove DO for master program -fyshi
!      DO

!       --- read and process user commands

        CALL SWTSTA(3)                                                    40.23
! master program change -fyshi
        if(Master_Start.eq.1)then
        CALL SWREAD (COMPUT)                                              40.31 30.90
        else
! master_time_ctr was modified from SWREAD in swanpre1.f -fyshi
        CALL MASTER_TIME_CTR(COMPUT)
        endif 
        CALL SWTSTO(3)     
        IF (STPNOW()) RETURN                                              34.01        
...
!           current, wind and friction
        ...
        IF(.NOT.ALLOCATED(COMPDA)) ALLOCATE(COMPDA(MCGRD,MCMVAR))         40.31
...
!           of using the S&L scheme                                       40.31
        ...
        IF ( NSTATM.EQ.1 .AND. MXITNS.GT.1 .OR. PROPSC.EQ.3 ) THEN        40.31
           IF (.NOT.ALLOCATED(AC1)) THEN                                  40.31
              ALLOCATE(AC1(MDC,MSC,MCGRD))
...
!  pass variables in swan  - fyshi

      call pass_in_swan(COMPDA,KGRPNT,XCGRID,YCGRID)


!fyshi     DO 500 IT = IT0, MTC                                           40.00

! test
      INDX=KGRPNT(10,10)
      print*,'-------', INDX,JVX2,COMPDA(INDX,JDP2)
...
!                 boundary differs from predescribed value given in       32.01
!                 boundary command values of incident HS are stored       32.01
!                 in HSI                                                  32.01
!
...
!
            ...
            ENDIF
!
            IF ( IT.EQ.IT0 .AND. .NOT.ALLOCATED(IONOD) ) THEN             40.31
               ALLOCATE (IONOD((MXMIP+1)*MXOURQ))                         40.30
               IONOD = -9999                                              40.30
            ENDIF                                                         40.00
!
            SAVITE = ITEST                                                30.21
...
!           --- carry out the output requests
     ...
     &                    KGRPNT, XCGRID, YCGRID, KGRBND, IONOD )         40.31
            CALL SWTSTO(9)                                                40.30
            IF (STPNOW()) RETURN                                          40.30
!
...
!
      CALL SWTSTO(1)                                                      40.23
!
!  skip close file if having master -fyshi
...
!         output requests in case of parallel computation                 40.30
      ...
      IF ( PARLL ) THEN                                                   40.30
         IF ( INODE.EQ.MASTER ) THEN                                      40.30
            ILEN = (MXMIP+1)*MXOURQ*NPROC                                 40.30
            ALLOCATE(IONODC(ILEN))                                        40.30
         END IF                                                           40.30
         CALL SWGATHER (IONODC, ILEN, IONOD, (MXMIP+1)*MXOURQ, SWINT )    40.30
         IF ( INODE.EQ.MASTER ) THEN                                      40.30
            CALL SWCOLOUT ( IONODC )                                      40.30
            DEALLOCATE(IONODC)                                            40.30
         END IF                                                           40.30
      END IF                                                              40.30
      CALL SWTSTO(9)                                                      40.30
!
...
!     --- deallocate all allocated arrays                                 40.31
      ...
      IF (ALLOCATED(IONOD )) DEALLOCATE(IONOD )                           40.30
      CALL SWCLME                                                         40.31
      print*,'down return'
!
      RETURN                                                              30.82
...
!
!     MCINGR increased by NUMGRD for array EXCVAL                         30.60
!     NMOVAR increased by 1, MXAUX increased by 3, NWL introduced         32.01
!     MCDA increased by 2, MCNMS increased by 1                           32.01
!     MCDA decreaded by 1 (JCGOB removed)                                 30.72
!     MCDA increased by 1 (JAUXW4 added)                                  30.70
!     NKTST increased by 1 (UNDFLW added)                                 30.72
!     MCCOM increased by 1 (SPDIR2 added)                                 30.75
!     MCDA  increased by 2 (JWFRCX and JWFRCY added)                      31.04
!     MCNMS increased by MQUAD (PQUAD added)                              34.00
!     NKTST increased by 1 (IFPAR added)                                  40.00
!     IDYNWL etc. removed from SWGRID, DYNDEP added                       40.00
!     MCNMS increased by 1 (BRESCL added)                                 40.00
!     LNAMS increased by 2 (DIRCH1 and DIRCH2 added)                      40.00
!     NLSDA LINELN (max length of input lines) introduced                 40.00
!     MCNMS increased by 1 (IGEN added)                                   32.06
!     MCNMS increased by 1 (URSELL added)                                 30.80
!     MNUMS increased by 5 (accuracy criteria for setup)                  30.82
!     MSETUP introduced                                                   30.82
!     MCNMS increased by 1 (CSETUP added)                                 30.82
!     MCNMS increased by 1 (ACUPDA added)                                 40.07
!     MNUMS increased by 5 (num. diffusion for SORDUP scheme)             33.10
!     MCNMS decreased by 1 (URSELL removed)                               40.03
!     MCDA  increased by 1 (JURSEL added)                                 40.03
!     MCNMS increased by 2 (MXITST, MXITNS added)                         40.03
!     NMOVAR increased by 1 (output quant. ASTD added)                    40.03
!     NUMGRD increased by 1 (input field ASTD added)                      40.03
!     MCFP   increased by 1 (CASTD added)                                 40.03
!     MCDA   increased by 3 (ASTD added)                                  40.03
!     MCINGR increased by 1 (VARAST added)                                40.03
!     LNAMS  increased by 20 (VERTXT added)                               40.03
!     MCDA   decreased by 1 (JASTD1 removed)                              40.14
!     MCODA  decreased by 31(several plot parameters removed)             40.31
!     MCCOM  decreased by 2 (JSIGMA and JSPDIR removed)                   40.31
!     MCUDA  decreased by 1 (MXOUTAR removed)                             40.31
!     MCDA   decreased by 26+MXAUX (several pointers in POOL removed)     40.31
!     MCNMS  increased by 51 (NCOMPT, RCOMPT added)
!
      PARAMETER (NMOVAR=48, NUMGRD=10, MSURF=10, MBOT=10,                 40.31 40.03
     &           MQUAD=10, MWCAP=15, MWIND=40, MTRIAD = 10, MNUMS=30,     40.07
     &           MSETUP=2,                                                30.82
     &           MSHAPE=5, MOUTPA=5, MICMAX=10, MSPPAR=5, LINELN=120)     40.00
      PARAMETER (LNAMS=574,
     &           NTSTD=5,                                                 40.00
     &           MCODA=5, NRFNS=12, NLSDA=19+2*LINELN, NLSDN=6,           40.31 40.00
     &           LHNAMS = 3*36 + 28 + 12*6 + NMOVAR*70,                   40.00
     &           MCINGR= 21+14*NUMGRD,                                    40.00
     &           MCCOM = 41, NKTST=13,                                    40.31 40.14
     &           MCNMS = MWCAP+MBOT+MTRIAD+MNUMS+MSURF+MWIND+MSHAPE+      40.00
     &                   MSETUP+MQUAD+MSPPAR+90,                          40.14 40.03
     &           MCUDA = 19+MOUTPA+NMOVAR*6, MCFP=18,                     40.31
     &           MCDA  = 63)                                              40.31 40.14
!
...
!     ***** numerical scheme *****
      ...
      ITRIAD = 0
!
! Quadruplets are now default on, since GEN3 KOM is default and the command
! GEN3 KOM did set the quadruplet source terms on.
!
      IQUAD  = 2                                                          30.72
...
!     *** Initial conditions ***
      ICOND = 0                                                          060697
!
!     *** Initial conditions for common SWNUMS                  ***       32.02
!
      BNAUT  = .FALSE.                                                    32.01
      BNDCHK = .TRUE.                                                     32.01
      BRESCL = .TRUE.                                                     40.00
...
!
!     *** setup flag ***                                                  32.02
!    setup =1 for calculate wave forcing -fyshi
      LSETUP = 1                                                          32.02
!
...
!     *** diffusion coefficient for explicit scheme ***
      PNUMS(9) = 0.01
!
!     *** parameters for the preconditioned BiCGSTAB solver    ***
!
!     *** PNUMS(10) : value indicating use of preconditioner   ***
!     ***   0  no preconditioner                               ***
...
!     ***  || Ax-b ||  <   eps1 + eps2 * || Ax - b ||          ***
!     ***                                     o                ***
!     ***  eps1 = PNUMS(11)   eps2 = PNUMS(12)                 ***
!
!     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!     !!!                                                      !!!
!     !!!  Note: since eps1 is not scale-invariant problems    !!!
!     !!!        may occur when action density and thereby     !!!
!     !!!        the residual is very small (much smaller      !!!
!     !!!        than eps1 = 1.e-6). As a consequence, the     !!!
!     !!!        solution will not be found since, the         !!!
!     !!!        termination criterion is satisfied!           !!!
!     !!!        This has been observed sometimes in non-      !!!
!     !!!        stationary computations. So, it is advice     !!!
!     !!!        to set eps1 = 0 in case of BiCGSTAB and in    !!!
!     !!!        particular for non-stationary runs!           !!!
!     !!!                                                      !!!
!     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!
!     *** PNUMS(14) : maximal number of iteration to be        ***
!     *** performed in each of the solution methods            ***
!
!     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!     !!!                                                      !!!
!     !!! Note: BiCGSTAB solver is relative expensive for a    !!!
!     !!!       relative simple matrix in spectral space,      !!!
!     !!!       which is penta-diagonal in case of currents    !!!
!     !!!       or instationary depth. Therefore, a fast       !!!
!     !!!       solver designed for penta-diagonal system of   !!!
!     !!!       equations is employed, called SIP developed    !!!
!     !!!       by H.L. Stone (1968). For this, PNUMS(11) is   !!!
!     !!!       set to -1.                                     !!!
!     !!!                                                      !!!
!     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
      ...
      PNUMS(14) = 20.
!
!     For the setup similar values can be used:
!     (BiCGSTAB solver will be used!)
!
!     PNUMS(22)~PNUMS(10); PNUMS(23)~PNUMS(12); PNUMS(24)~PNUMS(13)
!     PNUMS(25)~PNUMS(14)
!
      PNUMS(22) = -3.                                                     30.82
      ...
      PNUMS(25) = 20.                                                     30.82
!
! Maximum growth in spectral bin. Now has the value according to the default
! in the command GEN3 KOM
!
      PNUMS(20) = 0.1                                                     30.72
!
...
!
      PNUMS(18) = 0.8                                                     30.50
!
!     *** CFL criterion for ecplicit scheme in frequency space ***
!
      PNUMS(19) = 0.5 * sqrt (2.)
!
!     ADJUSTABLE DIFFUSION UNLIKELY TO EVER BE NECESSARY (PNUMS(26))      40.08
!
...
!
      ...
      PTRIAD(5)  = 0.01                                                   40.23
!
!     *** if the Lumped Triad Approximation of Eldeberky (1996) is used    rr
!     *** the coefficient for PTRIAD(1) should be set equal to 0.25          rr
!     **** moreover: the coefficient to calculate the maximum discrete      rr
!     *** is stored in PTRIAD (2)  --> value has to be 2.5                   rr
!     PTRIAD(1) = 0.25                                                       rr
!     PTRIAD(2) = 2.5                                                        rr
!
...
!
     ...
     &                FSHAPE, DSHAPE)                                     40.31
      ...
      END DO                                                              40.31
!
!     --- deallocate global arrays                                        40.31
!
      IF ( NSTATM.EQ.0 .AND. NSTATC.EQ.0 ) THEN                            40.31
         IF (ALLOCATED(XGRDGL)) DEALLOCATE(XGRDGL)                        40.31
         IF (ALLOCATED(YGRDGL)) DEALLOCATE(YGRDGL)                        40.31
         IF (ALLOCATED(KGRPGL)) DEALLOCATE(KGRPGL)                        40.31
         IF (ALLOCATED(KGRBGL)) DEALLOCATE(KGRBGL)                        40.31
      END IF                                                              40.31
!
      RETURN
...
!
          IF (VARAST) THEN
             ASTD = SVALQI (XP, YP, 9, ASTDF, 1 ,IX ,IY)                  40.31 40.03
...
!     --- A warning about curvilinear and S&L scheme                      40.08
      IF ((PROPSC.EQ.3).AND.(OPTG.EQ.3)) THEN                             40.08
         ...
         CALL MSGERR(1,'and differences are usually rather miniscule.')   40.08
      ENDIF                                                               40.08
!
!     Here the various problems with quadruplets are checked
!     The combination of quadruplets and sectors is an error in the calculation
!     of Quadruplets when the SECTOR option is used in the CGRID command. This
!     error should be corrected in the future
!
...
!       Add bottom level to obtain depth
 ...
 41       CONTINUE
 31     CONTINUE
c  test if water level pass to swan
c         print*,COMPDA(KGRPNT(10,10),JWLV2),COMPDA(KGRPNT(10,10),JDP2)
c     &         ,COMPDA(KGRPNT(10,10),JDP1)
        IF (NPTST.GT.0) THEN
...
!  2. Purpose
!
!     read boundary spectra from one file and aditional information of the
!     heading lines
!
!  3. Methode
!
...
!     COUNT_IT  counter of the time entering in the boundary file
!
...
!     DUM_A     dummy local varable (not used in any calculation)
...
!
!     if time > time of last set of spectra, read new spectra
!
...
!
!         read aditional information from the headers and spectrum from
...
!           instead of radians (Swan 2D spectral files)
            ...
            ELSE                                                          40.05
              UFAC = 1./ (2.*PI)
            ENDIF
!           divide by Rho*Grav if quantity on file is energy density
...
!                    convert time string to time in seconds
                  ...
                  ELSE                                                    40.05
!                   DUM_A dummy local varable used to read  formated files40.05
!                   this varible will be not used in any calculation      40.05
...
!
        ELSE                                                              40.05
!       if time < time of last set of spectra then transform to spectral
!       resolution used in SWAN to obtain boundary spectra
!
...
!     40.05, Aug. 00: WW3 nesting and changes in the form of the code
!                     (use of f90 features), Revise vertion of subroutine
...
!  2. Purpose
!
!     read one 1-d OR 2-d boundary spectrum from file, and transform
...
!
!  7. Common Blocks and Modules Used
!
!
!  8. Subroutines used
!
...
!         results appears in array BAUX2(MDC,NFRE)
!
        DO IFRE = 1, NFRE                                                40.05
...
! 13. Source text
!
      IF (ALLOCATED(SPCSIG_f90)) DEALLOCATE(SPCSIG_f90)
