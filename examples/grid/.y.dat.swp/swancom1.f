...
!     SOLBAND (solve penta-diagonal system (due to implicit scheme in     40.00
!              frequency space) with an iterative ILU-BiCGSTAB solver)
...
!
!----------------------------------------------------------------------
!     The following subroutines can be found in the file : SWANCOM2.FTN
!
!     SBOT    (Bottom friction )
!     FRABRE  (Fraction of breaking waves)
!     FRABRE2 (Fraction of breaking waves, smoother version of FRABRE)    30.77
!     SSURF   (Wave breaking: Battjes and Janssen (1978) )
!     SWCAP   (Whitecapping; five formulations)                           40.02
!     BLOCKE  (remove action from spectrum for explicit calculation)
!     BRKPAR  (determine slope depended breaking parameter according
!              to Nelson (199?)
!     CNTAIL  (contributions to the spectrum of the high frequency
!              tail)
!     PLTSRC  (store the values for plot of the source terms and spec.)
!
!----------------------------------------------------------------------
!     The following subroutines can be found in the file : SWANCOM3.FTN
!
...
!     SWIND0 (linear input term Cavaleri and Malanotte Rizolli )
!     SWIND1 (first generation wind growth model (Dolphin formulation)
!     SWIND2 (second generation wind growth model (Dolphin formulation)
!     SWIND3 (third generation wind growth model (Snyder et al. 1981;
!             Komen et al., 1984)
!     SWIND4 (third generation wind growth model (Janssen, 1989,1991)
!     NEWTON (determine the new friction velocity for the Janssen
!             growth fromulation with an iterative Newton-Raphson method
!
!----------------------------------------------------------------------
!     The following subroutines can be found in the file : SWANCOM4.FTN
!
...
!
!---------------------------------------------------------------------
!     The following subroutines can be found in the file : SWANCOM5.FTN
!
!     SWPSEL   ( determine spectral counters in presence
!                or absence of a current )
!     SPROXY  ( compute propagation module CAX, CAY )
!     SPROSD  ( compute propagation module CAS, CAD )
!     DSPHER  ( compute Ctheta for propagation over the globe )           33.09
!     STRSXY  ( compute derivative in space and time )
!     SORDUP  ( compute spatial derivatives with 2nd order scheme )       33.10
...
!     STRSS1  ( compute derivative in s-space according to TOLMAN )
...
!
!----------------------------------------------------------------------
!     The following subroutines can be found in the file : SWANCOMI.FTN
!
!     CGSTAB   Solve an unsymmetric system of linear equations
!              by the BiCGSTAB method. The subroutine contains
...
!     ****************************************************************
!
!     The main program structure of SWAN with its subroutines
!     is as follows :
!
!     +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!     +                                                               +
...
!     +  =============                                                +
!     +              |                                                +
!     +              |---- SOPCRE, SOPIDS, SOPLOT                     +
!     +              |                                                +
!     +              |---- SWANPRE1, SWANPRE2                         +
!     +              |                                                +
!     +              |---- SWANCOM1, SWANCOM2, SWANCOM3, SWANCOM4,    +
!     +              |     SWANCOM5                                   +
!     +              |                                                +
!     +              |---- SWANOUT1, SWANOUT2, SWANOUT3               +
!     +                                                               +
!     +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
!
...
!     several subroutines.
!     The input for this subroutine comes from SWANPRE1, SWANPRE2,
!     and SWANPRE3. The output is send to the subroutines SWANOUT1,
!     SWANOUT2 and SWANOUT3. The output consist of some characteristic
...
!     higher order upwind schemes, flux limiting, BiCGSTAB solver,
!     SIP solver
!
...
!     CG        3D    Group velocity as function of IC and IS and D in the
!                     direction of wave propagation in presence of currents
...
!     HSACC1    2D    Represent the significant wave height at time T
!     HSACC2    2D    Represent the significant wave height at time T+1
...
!     SACC1     2D    Represents the mean wave period at time T
!     SACC2     2D    Represents the mean wave period at time T+1
...
!                     no current <---> current
!     SECTOR    1D    Indicates which configuration is present (see
!                     subroutine SWPSEL )
...
!     COLU2     1D    In presence of a current the spectral direction can
!                     be circular and closed. Matrix coefficients appear in
!                     the top right and bottom left corner of the matrix
!                     After pivoting --> coefficients are stored in COLU2
!                     space
!     DIFLOW    2D    Lower diagonal in solver for diffsion
!     DIFDIG    2D    Main diagonal in solver for diffusion
!     DIGUPP    2D    Upper diagonal in solver for diffusion
!     DIFRHV    2D    Right hand side for diffusion
...
!     PSURF(1)  = ALFA     1.0      (Battjes Jansen)
...
!     PNUMS(10)  = PREC    type of preconditioner meant for BiCGSTAB solver
!     PNUMS(11)  = EPS1    >0: termination criterion in absolute sense meant
!                              for BiCGSTAB solver
!                          <0: SIP solver will be used instead of BiCGSTAB
...
!
!     WWINT ( 1 = IPD    WWAWG ( = AGW1    WWSWG ( = SWG1
!             2 = IDP1           = AWG2            = SWG2
!             3 = IDM            = AWG3            = SWG3
!             4 = IDM1           = AWG4            = SWG4
!             5 = ISP            = AWG5            = SWG5
...
!
!     1. SPRE      (preparation of the computation (reading parameters))
!     2. SWCOMP    (computation of the action densities (discussed below))
!     3. SOUT      (output of the program)
!
!     In this part the subroutine SWCOMP is discussed:
!
!     SWCOMP                                 Sweeps: loop over spatial points
!     ======
!      |
!      |------+  INSAC                       determine initial values for
!      |                                     accuracy check
!      |
!      |-------+ SINSTR                      read incoming waves (this
!      |                                     subroutine should be implemeted
!      |                                     in the SWANMAIN instead of in
!      |                                     SWANCOM1
!      |
!      |
...
!      |       |        |
!      |       |        +-------+ FRABRE2    comp. of fraction of         30.77
!      |       |                             breaking waves
...
!      |       |        +-------+ SSURF      wave breaking
!      |       |        |
!      |       |        +-------+ STRIAD     nonlinear wave interactions
!      |       |        |
...
!      |
!      |-------+ SACCUR                      check accuracy of the comp.
!      |
!     END SWCOMP
!
!
! 12. Structure
!
!     The general numerical procedure in SWAN is based on the next
!     principle of sweeps (see Holthuijsen et al 1993) where
!
!
!       {**************************************************************}
...
!
!       Call SINSTR to read incoming waves at the boundaries (N,E,S,W) in
!                   stationary and nonstationary case  SINSTR changes
!
!
...
!     ----------------------------------------------------
!     CALL SACCUR to check the accuracy of the computation
!     --------------------------------------------------------
!     End of SWCOMP
!     --------------------------------------------------------
!
...
!
     ...
     &                     ' for spatial propagation (SANDL).'            33.08
          ICMAX  = 10                                                     33.08
        ELSE IF (PROPSC.EQ.2) THEN                                        33.10
          WRITE(PRINTF,*) 'The 2nd order upwind scheme will be used ',    33.10
     &                    'for geographic propagation (SORDUP).'          33.10
          ICMAX  = 5                                                      33.10
        ELSE
          WRITE(PRINTF,*) 'The 1st order upwind scheme will be used ',    33.08
     &                    'for geographic propagation (BSBT).'            33.08
          ICMAX  = 3                                                      33.10
        ENDIF                                                             33.08
...
!
 ...
 4113   FORMAT(' 1) Implicit scheme   : CSS    ',E12.4)
        IF ( PNUMS(11).LT.0. ) THEN                                       40.23
           WRITE(PRINTF,*) 'Solver is SIP'                                40.23
        ELSE                                                              40.23
           WRITE(PRINTF,*) 'Solver is BiCGSTAB'                           40.23
        END IF                                                            40.23
 ...
 4213   FORMAT(' 2) Explicit scheme   : CFL    ',E12.4)
!
...
!
     ...
     &                   )  THEN
      ...
      END IF
!
!     *** check resonance condition for triads ***
!
...
!
 ...
 121    CONTINUE
      ENDIF
!
!     *** calculate auxiliary variables MSCMAX and MDCMAX   ***
!     *** for the 4-wave interactions to allocate required  ***
!     *** memory in the WAREA (The size of the arrays for   ***
!     *** the quadrupltes are on the forehand not known     ***
!
...
!
!     in case of SETUP expand array for setup data                        31.02
...
!$OMP+PRIVATE(UPPERI, LOPERI, INOCNT)                                     40.31
!$OMP+PRIVATE(RINSOL, INFMAT, IINSOL)                                     40.22
!$OMP+PRIVATE(IP,IDC,ISC)                                                 40.31
!$OMP+PRIVATE(I1GRD,I2GRD,I1MYC,I2MYC)                                    40.31
!$OMP+PRIVATE(IARR,ARR,JDUM,JJ,III)                                       40.31
...
!       *** no quadruplets ***
        ...
        ALLOCATE(DSNL(0,0))                                               40.22
      END IF
!     *** explicit scheme in frequency space and diffusion model ***
...
!       *** BiCGSTAB solver ***
      ...
      END IF
!
!     *** wind input term Janssen (1989, 1990) ***
!
...
!----------------------------------------------------------------------   40.31
      ...
      DIFUPP = 0.                                                         40.31
      DIFRHV = 0.                                                         40.31
      IF ( DYNDEP .OR. ICUR.EQ.1 ) THEN                                   40.31
        BAND   = 0.                                                       40.31
        EXACT  = 0.                                                       40.31
        RHV    = 0.                                                       40.31
        SOLUT  = 0.                                                       40.31
...
!
!
!     *** store original wind model counter (IWIND) in auxiliary ***
!     *** variable IWORG (required for first guess)              ***
!
!$OMP MASTER                                                              40.31
      IWORG = IWIND
...
!       initialise local (thread private) counter for BiCGSTAB/SIP solver 40.31
        INOCNT = 0                                                        40.31
!
!       initialise Dissipation and Leak to 0 at begin of iteration
!       this is done in parallel within OpenMP environment                40.31
!
        ...
        ENDDO
!
!       initialise Ursell number to 0                                     40.03
!       this is done in parallel within OpenMP environment                40.31
!
        IF (ITRIAD.GT.0) THEN
           DO IP = I1GRD,I2GRD                                            40.31
...
!           *** new iteration                                  ***        40.31
!           *** This is done in parallel within OpenMP environ ***        40.31
!                                                                         40.31
        IF ( IQUAD .GE. 3 ) THEN                                          40.31
          DO IP = I1GRD,I2GRD                                             40.31
            DO ISC = 1,MSC                                                40.31
...
!                                                                         40.31
        INOCNV = 0                                                        40.31
!
!       *** prepare constants and weight factors for nonlinear   ***
!       *** 4 wave interactions and initialize array MEMNL4 = 0. ***
!       *** and set the array for the frequencies > SPCSIG(msc)  ***      30.72
!
        CALL SWTSTA(135)                                                  40.23
        IF ( IQUAD .GE. 1 ) THEN
!
!       *** For the first iteration compute range for spectral     ***
!       *** space and some other variables for 4 wave interactions ***
!
          IF ( ITER .EQ. 1 ) THEN
              CALL FAC4WW (ITER  ,XIS   ,SNLC1 ,                          34.00
     &                    DAL1  ,DAL2  ,DAL3                ,SPCSIG,      34.00
     &                                          WWINT              ,      40.17 40.02 40.22
...
!         *** of active wind ) in a second generation mode first.  ***    40.00
!         *** After 1 iteration, the options as defined by the     ***
!         *** user are activated.                                  ***
!         *** This first guess is not used in nonstationary        ***    40.00
...
!           *** third generation wave input ***
            IF ( ITER .EQ. 1 )THEN
!             *** save settings of 3d generation model              ***   40.13
!             *** bottom friction and surf breaking are not changed ***
              KSTATC = NSTATC
              KCUR   = ICUR
!             *** deep water model ***
              KWIND  = IWIND
...
!             *** shallow water model ***
              KTRIAD = ITRIAD
!             ***  maximum change per bin is PNUMS(20)  ***               40.13
              GRWOLD = PNUMS(20)
...
!           make LLOCK False for points on boundary                       40.22
            IF (SWPDIR.EQ.1) THEN
              ...
              IF (KREPTX.EQ.0) THEN
                IX1 = 2
                IF (.NOT.LMXF) IX1 = IX1-1+IHALOX                         40.31
!$              LLOCK(IX1-1,:) = .FALSE.                                  40.31 40.22
              ...
              IY1 = 2
              IY2 = MYC
              IF (.NOT.LMXL) IX2 = IX2-IHALOX                             40.31
              IF (.NOT.LMYF) IY1 = IY1-1+IHALOY                           40.31
              IF (.NOT.LMYL) IY2 = IY2-IHALOY                             40.31
!$            LLOCK(:,IY1-1) = .FALSE.                                    40.31 40.22
            ELSE IF (SWPDIR.EQ.2) THEN
              ...
              IF (KREPTX.EQ.0) THEN
                IX1 = MXC-1
                IF (.NOT.LMXL) IX1 = IX1+1-IHALOX                         40.31
!$              LLOCK(IX1+1,:) = .FALSE.                                  40.31 40.22
              ELSE
                IX1 = MXC
              ENDIF
              ...
              IY1 = 2
              IY2 = MYC
              IF (.NOT.LMXF) IX2 = IX2+IHALOX                             40.31
              IF (.NOT.LMYF) IY1 = IY1-1+IHALOY                           40.31
              IF (.NOT.LMYL) IY2 = IY2-IHALOY                             40.31
!$            LLOCK(:,IY1-1) = .FALSE.                                    40.31 40.22
...
!$              LLOCK(IX1+1,:) = .FALSE.                                  40.31 40.22
              ...
              IY2 = 1
              IF (.NOT.LMXF) IX2 = IX2+IHALOX                             40.31
              IF (.NOT.LMYL) IY1 = IY1+1-IHALOY                           40.31
              IF (.NOT.LMYF) IY2 = IY2+IHALOY                             40.31
!$            LLOCK(:,IY1+1) = .FALSE.                                    40.31 40.22
...
!     in which the BiCGSTAB/SIP solver did not converge.                  40.31
!----------------------------------------------------------------------   40.31
!$OMP ATOMIC                                                              40.31
...
!         arrays and no longer using WAREA.                               40.22
          CALL SWTSTA(102)                                                40.23
          CALL SACCUR (COMPDA(1,JDP2),KGRPNT          ,XYTST           ,  40.30
...
!$OMP MASTER                                                              40.31
!
!         *** info regarding the iteration proces and the accuracy ***
!
...
!
!       Print message when BiCGSTAB solver did not converge
!       in setup calculation
!
 ...
 123    FORMAT(2X,'BiCGSTAB solver: no convergence in ',I4,
     &            ' gridpoints')
 124    FORMAT(1X,'BiCGSTAB solver: no convergence for setup')            30.82
!
        CALL SWTSTA(105)                                                  40.23
...
!       {to reduce the size of the arrays K, CG, CPX, CPY, CAX, CAY, CAS, CAD
...
!                    Cgo and Cg in wave direction (currents included)
...
!         Call SPREDT to predict the action density taking into account
!                     the propagation direction of the waves and the two
!                     adjecent points
!       -----------------------------------------------------------------
...
!           else if BiCGSTAB solver do
!              Call SOLBAND  transform 2D matrices to 1D arrays and
!                            solve penta-diagonal system by means
!                            of BiCGSTAB solver with an ILU-type
!                            preconditioner
!         -----
!         endif
...
!
!     ***  if (kcgrd(ic) <= 1)  return (point is NOT A sea point)  ***
      IF (KCGRD(1) .LE. 1 .OR. COMPDA(KCGRD(1),JDP2) .LE. DEPMIN)RETURN   25/MAR
!
!     Bugfix dated July 13, 1998; prevents transport from a permanent land
!     point (having address 1), only effective if Exception values are used
!     in reading depth and/or coordinates
!
      IF (KCGRD(2) .LE. 1) RETURN                                         30.70
      IF (.NOT.ONED .AND. (KCGRD(3).LE.1)) RETURN                         30.70
!      can be replaced later by: IF(PROPSL.EQ.0)RETURN                    33.09
!
!
...
!         calculate CAX1 and CAY1, in case of S&L scheme                  33.08
...
!
!         if nonstationary, then cax1 is not necessarily = cax.           33.08
...
!     ..........JDP1 used rather than JDP2.                               33.08
!     ..........JVX1 used rather than JVX2, JVY1 used rather than JVY2    33.08
!     ..........this modified code has not been validated with            33.08
!     ..............nonstationary currents, but i don't expect any        33.08
!     ..............problems with it.                                     33.08
!
...
!     ................JVX1,JVY1 are used rather than JVX2,JVY2.           33.08
!
        END IF                                                            33.08

!       calculate propagation velocities for new time level               40.13

!       *** Compute wavenumber and group velocity CGO and the ***
!       *** groupvelocity CG = CGO + U for all directions in  ***
!       *** the gridpoints of the stencil                     ***
...
!       *** in the central gridpoint only ( IC = 1 ) for   ***
!       *** those directions which lie within a sweep      ***
!
!          due to transfer of loop over IC IF statement is not needed     40.13
!
!       *** compute minimum and maximum counter (IDCMIN and ***
!       *** IDCMAX) and determine the type of SECTOR and    ***
!       *** fill the array ANYBIN to determine if a bin     ***
!       *** lies within the sweep considered                ***
!
...
!
!
!         *** predict action density for first iteration using the ***
!         *** two adjacent gridpoints                              ***
!
...
!
              CALL SWTRCF (CROSS, COMPDA(1,JWLV2),                        40.31 40.09
...
!
        END IF
!
!       *** compute contribution of the high frequency tail  ***
!       *** to the energy spectrum only for "real" cases     ***
!       *** for academic tests with only 2 bins in frequency ***
!       *** space (MSC = 3) the contributions are all set 0. ***
!
!
...
!
         ...
         ENDIF
!
!        *** compute the total action density in a point  ***
!        *** depth dependency is in the subroutine itself ***
!
!
...
!
!       *** start computation of action density for every frequency ***
!
...
!           *** diagonal system with the SIP solver or the       ***
!           *** BiCGSTAB solver with an ILU-type preconditioner  ***
!
            CALL SWTSTA(120)                                              40.23
            IF ( PNUMS(11).LT.0. ) THEN
...
!         *** Solve the tri-diagonal matrix with Thomas algoritm     ***
!
          CALL SWTSTA(120)                                                40.23
...
!
!       NOTE: if PTRIAD(3) = 0.1, then the model results will be          40.31
!             different from SWAN version 40.11 !!!                       40.31
!             It is to advise to always activate the limiter              40.31
!             independent of the status of Snl4, i.e. active if           40.31
!             Ur<PTRIAD(3) and non-active if Ur>PTRIAD(3)                 40.31
!             Therefore, the change 40.16, i.e. no limiting if            40.31
!             also Ur>PTRIAD(3), has been withdrawn                       40.31
!
...
!
!       *** set action density in land points equal to zero ***
!
        DO IS = 1, MSC
          DO ID = 1, MDC
...
!        ----------
!
!        WETGRD      Counter for the gridpoints which have a depth > 0.
!                    ( wet gridpoints )
!        IX          Counter of gridpoints in x-direction
!        IY          Counter of gridpoints in y-direction
!        IS          Counter of relative frequency band
!        ID          Counter of the spectral direction
!        ICMAX       Maximum counter for the points of the stencil
...
!        PERCNT      Dummy counter for the gridpoints where the
!                    accuracy has not been reached
!        HSRELF      Computed relative error in the significant wave height
!                    according to the value at the old time level
!        RFAULT      Maximum acceptable relative error in the significant
!                    waveheight and the mean frequency in a gridpoint
!        SRELF       Computed relative error in the mean frequency
!                    according to the value at the old time level
!        SMA_T       Dummy variable
!        SMA_B       Dummy variable
!        HMA_T       Dummy variable
!        HMA_B       Dummy variable
!
!
!        one and more dimensional arrays:
!        ---------------------------------
...
!        DEP2      2D    Depth
!        HSACC1    2D    Dummy array for the significant wave height
!                        (old value)
!        HSACC2    2D    Dummy array for the significant wave height
!                        (new value)
!        SACC1     2D    Dummy array for the mean frequency (old value)
!        SACC2     2D    Dummy array for the mean frequency (new value)
!        DELHS     2D    difference in Hs between last 2 iterations
!        DELTM     2D    difference in Tm between last 2 iterations
!
!        PNUMS(1)  = DREL     relative error in Hs and Tm
!        PNUMS(2)  = DHABS    absolute error in Hs
!        PNUMS(3)  = DTABS    absolute error in Tm
!        PNUMS(4)  = NPNTS    number of points where accuracy is reached
!
!  6. Local variables
!
!     INDX  : counter                                                     30.82
...
!   ---------------------------------------------------------------
!   If relative fault for mean frequency or significant wave height
!      > certain given value then increase variale with one and
...
!  2. Purpose
!
!     to determine the transportation, refraction and the source terms
...
!                     a current (see routines SOLBAND or SWSIP)
...
!
!     ---------------------------------------------------------
!     Call STIME to compute the time derivative (@AC2/@t) using the values
!                of the action density of the last time step
!     -------------------------------------------------------------------
...
!     endif
!     -------------------------------------------------------------------
!     Call STRSS1 to compute the propagation terms in frequency
!                 space (this method is based on the numerical procedure
!                 as described in Tolman 1991 and should not be used in
!                 SWAN since it is based on a non stationairy situation
!                 (limiting with time step )
!     -------------------------------------------------------------------
...
!
!       *** If there are obstacles crossing the points in the stencil ***
!       *** then the transmission and reflection coeff. are computed  *** 30.70
!
        IF (SWPDIR .EQ. 1 ) THEN
          LINK(1) = CROSS(1,KCGRD(1))
          LINK(2) = CROSS(2,KCGRD(1))
 ...
 10       FORMAT(' ACTION:  SWPDIR POINT LINK1  LINK2 = ',4(1X,I5))
!
          CALL SWTRCF (CROSS, WLEV2, CHS, LINK, OBREDF                    40.31 40.09
...
!                               2
!     KM_WAM  = (ETOT / EDRKTOT)
!     QB      : Using the subroutine FRABRE
!     SIGM01  = ETOT1 / ETOT
!     SIGM_10 = ETOT  / ACTOT
!     UBOT    = Sqrt [ UB2 ]
!
...
!
      ...
      UBOT(KCGRD(1)) = 0.
!
!     Calulate total spectral energy:
!
!
...
!
!     Calculate the orbital velocity UBOT and orbital excursion ABRBOT
!
...
!
!     NOTE: this 40.16-implementation will slightly change the model      40.31
!           results obtained with SWAN version 40.11 !!!                  40.31
!
...
!
      DO IS = 1, MSC
        DO ID = 1, MDC
          IF ( SECTOR(IS).EQ.0 .OR. .NOT.ANYBIN(ID,IS) ) THEN
            IMATLA(ID,IS) = 0.
            IMATDA(ID,IS) = 1.
            IMATUA(ID,IS) = 0.
...
!     30.80, July 99: call ISSOLV modified
!
!  2. Purpose
!
!     Solve the array in case of a current. A fully implicit scheme
!     in frequency and disrectional space is used.
!
!     The subroutines that solve this matrix have been provided
!     by Dr. C. Vuik from Delft University of Technology, the
!     Netherlands.
!
!        /                                         \
!       | 1  6        7  8  9        U             |
...
!       \                                          /
!
!     In this model the diagonals: 1 , 3 , 5 , 6 , 8 , L and U
!     are used.
!
!  3. Method
!
...
!        MDC         Maximum counter of directional distribution
!                    one sweep
!        IDTOT,ISTOT maximum range between the minimum and maximum
!                    counter in directional and frequency space
!                    respectively
!
!        REALS:
!        ---------
...
!        IDCMAX    1D    Integer array containing maximum counter
!        ANYBIN    2D    Logical array. If a certain bin is enclosed
!                        in a sweep then ANYBIN is TRUE. This array is
!                        used to determine whether some coefficients
!                        in the array have to be changed
!        IMATDA    2D    Coefficients of diagonal of matrix
!        IMATLA    2D    Coefficients of lower diagonal of matrix
...
!************************************************************************
!
!     NX replaced by NSC (NX is a common variable)                        40.00
...
!
!     *** ERROR on minimum values of ISTOT and IDTOT ***
!
      IF (  ( ISTOT .LT. 4 .AND. IDTOT .LT. 3 ) .OR.
     &      ( ISTOT .LT. 3 .AND. IDTOT .LT. 4 ) ) THEN
        CALL MSGERR(3,'vector length too small for BiCGSTAB solver')      40.13
...
!
      ...
      NWORK  = 10*NTOT
!
!     *** Number of finite volumes in x-direction in NSC = (MSC-1) ***
!
      NSC = ISTOT - 1
!
!     *** set all values in arrays equal 0. ***
!
      DO I = 1, MSCMDC
        DO J = 1, 9
          BAND(I,J)   = 0.
...
!
!     infmat(1) = 4 represents the original equation (spectral plane)     30.80
!     infmat(1) = 5 represents the Poisson equation (setup)               30.80
      INFMAT(1) = 4
      INFMAT(2) = NSC
      INFMAT(4) = 0
!
      ...
      IF ( FILL ) THEN
!
!       *** fill solution array with value of previous iteration ***
!       *** in area from 1 to ISSTOP and IDDLOW to IDDTOP        ***
!
        DO IDDUM = IDDLOW, IDDTOP
...
!
      ELSE
!
!       *** fill matrix with predefined coefficients to test     ***
!       *** the BiCGSTAB solver                                  ***
!
!       *** fill solution array with value of previous iteration ***
!
...
!     ***      2: additional information about the iteration   ***
!     ***         is printed                                   ***
!     ***      3: gives a maximal amount of output             ***
!     ***         concerning the iteration process             ***
!
!      IINSOL(3) = 2
      IINSOL(3) = INT( PNUMS(13) )
!
!     *** IINSOL(6) maximal number of iteration to be performed ***
...
!
     ...
     &                  ITSW    ,ITER    )                                30.72
      IF (STPNOW()) RETURN                                                34.01
!
!     *** fill action density array with solution obtained from ***
...
!        IDCMIN    1D    Integer array containing minimum counter
!        IDCMAX    1D    Integer array containing maximum counter
!        SECTOR    1D    sectors inclosed in a sweep.
!        ANYBIN    2D    Logical array. if a certain bin is enclosed
!                        in a sweep then ANYBIN is TRUE . array is
!                        used to determine whether some coefficients
!                        in the array have to be changed
!        ICOLU2    1D    auxiliary array for storing the coefficients
!                        in the last column
!
!     5. SUBROUTINES CALLING
!
...
!
!     *** if explcit scheme is used and CFL exeeds CFL max ***
!     *** then bin should not be propagated within a sweep ***
!
      DO IS = 1, ISSTOP
        DO IDDUM = IDDLOW, IDDTOP
          ID = MOD ( IDDUM - 1 + MDC , MDC ) + 1
...
!
          ...
          ENDDO
!
!         *** if SECTOR(IS) = 4 then four sectors are enclosed.     ***
!         *** change the coefficients in the matrix for those       ***
!         *** rows for which the array if ANYBIN(IS,ID)             ***
!         *** is FALSE                                              ***
!
...
!  2. Purpose
!
!     to compute the source terms, i.e., bottom firction,
...
!     GRAV        Gravetational acceleration
!     HM          Maximum wave height
...
!     6. SUBROUTINES USED
!
!        SBOT    ,SSURF   ,SWIND1 ,SWIND2 ,SWIND3 ,SWIND4,
!        SWCAP   ,                                                        40.02
...
!
!   ------------------------------------------------------------
!   If SBOT is on (IBOT > 1 ) then,
!     Call SBOT  to compute the source terms due to bottom friction
!      according to Hasselmann et al. (1974), Putnam and Jonsson (1949)
!      or Madsen et al. (1991)
!   ------------------------------------------------------------
!   If SSURF is on (ISURF > 1 ) then,
!     Call SSURF to compute the source terms due to wave breaking
!       according to Battjes and Janssen (1978)
!   ------------------------------------------------------------
!   IF IWIND =1 OR IWIND =2 THEN
!     Call WNDPAR (first or second generation first guess of the spectrum  RR
!                  using the DOLPHIN-B formulations)                       RR
!
...
!     for low frequencies and to Plant's (19??) form for high freq.
!     Call SWIND5
!   ------------------------------------------------------------
!   If IWCAP > 1 then
!     Call SWCAP to compute the source term for white capping             40.02
!   ---------------------------------------------------------------------
!   If STRIAD is on (ITRIAD = 1 ) then,                                   30.80
!   Then Call STRIAN to compute the nonlinear 3 wave-wave interactions
!        according to Eldeberky and Battjes (1995)
!   ---------------------------------------------------------------------
!   If triads are not active                                              30.80
!   Then If Squad is on (IQUAD = 1 )
...
!             Call FILNL3   Get values for interactions from array
!             for full circle
!   ---------------------------------------------------------------------
!
!     10. SOURCE
...
!     Initialisation of MEMNL4                                            40.17

      IF ((ITER.EQ.1).AND.(IT.EQ.1).AND.(IQUAD.GE.3)) THEN                40.17
        MEMNL4(:,:,KCGRD(1)) = 0.                                         40.17
      ENDIF                                                               40.17
!
...
!
     ...
     &                 ) THEN
!
!       *** linear wind input according to Cavaleri and Melanotte ***
!       *** Razolli (1981) for a third generation mode of SWAN    ***
!
...
!
!
!     Calculate whitecapping source term (five formulations)              40.02
!
...
!       *** geographical gridpoint a continues spectrum     ***
!       *** present is, i.e., after one iteration           ***
!
...
!
!     compute quadruplet interactions if triads are not active:           30.80
!
      CALL SWTSTA(135)                                                    40.23
      IF (URSELL(KCGRD(1)).LT.PTRIAD(3)) THEN                             40.03
!
!       *** compute the counters for the nonlinear 4ww- ***
...
!           *** point for the first iterations to ensure stable     ***
...
!         Multiple DIA according to Hashimoto (1999)                      40.17
     ...
     &        ( SWPDIR .EQ. 4 .AND. (IX.EQ.MXC .AND. IY.EQ.1))))) THEN
            DO IDIA=1,MDIA                                                40.17
              ...
              CALL FAC4WW (ITER  ,XIS   ,SNLC1 ,                          40.17
...
!           divide by sigma to get action density source term             40.33
 ...
 9200             FORMAT(' SOURCE: IS ID MEMNL(): ',2I6,E12.4)            40.10
          ...
          CALL FILNL3 (MDC     ,MSC     ,IDCMIN  ,IDCMAX  ,IMATRA  ,      40.10
     &                 IMATDA  ,AC2     ,                                 40.23
     &                 MEMNL4  ,PLNL4S  ,ISSTOP  ,KCGRD   ,MCGRD   ,      40.10
     &                 ICMAX                                       )      40.10
...
!     --- construct L and U matrices (stored in CMAT[xx])
      ...
      DO IDDUM = IDCMIN(IS)+1, IDCMAX(IS)
         ID  = MOD ( IDDUM - 1 + MDC , MDC ) + 1
...
!         in an iterative manner
 ...
 10   IF ( ICONV.EQ.0 .AND. IT.LT.MAXIT ) THEN
     ...
     &            ' ++ SWSIP: iter = ',IT,'    res = ',RNORM
         ...
         DO IDDUM = IDCMAX(IS)-1, IDCMIN(IS), -1
            ID  = MOD ( IDDUM - 1 + MDC , MDC ) + 1
            IDP = MOD ( IDDUM     + MDC , MDC ) + 1
