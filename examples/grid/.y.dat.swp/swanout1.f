...
!
! i   IONOD : array indicating in which subdomain and at what time        40.30
...
!
      INTEGER IONOD(MXMIP+1,MXOURQ)                                       40.30
...
!
     ...
     &               SNAME, NVOQP, OQPROC, BKC, VOQR, IONOD(1,IRQ),
     &               LOGACT)                                              30.00
...
!
     ...
     &               COMPDA, KGRPNT, IONOD(1,IRQ))                        40.31
!
        IF (BKC .GT. 0) THEN
!
...
! test pass direction fyshi
             call pass_out_swan(COMPDA,KGRPNT,XCGRID,YCGRID,
     &            MXK,MYK,VOQR,VOQ(1))
...
!       ***** block output *****
     ...
     &                     SNAME, MXK, MYK, VOQR, VOQ(1) )                40.31
          END IF                                                          40.31
...
!                             (subscript: IVTYP)
!     IONOD   Int ar input    array indicating in which subdomain and     40.30
!                             at what time output points are located      40.30
!
!  8. Subroutines used
!
!     SPSET
...
! 13. Source text
!
      INTEGER    VOQR(*), OUTI(*), BKC, IONOD(*)                          40.30 30.00
      INTEGER    IVTYP(*)                                                 40.31
      REAL       OUTR(*)                                                  30.00
      LOGICAL    OQPROC(NMOVAR), LOGACT                                   30.00
...
!       computational period and the user do not ask for a periodic plots
        DIF = TFINC - TIMCO
        TNEXT = OUTR(1)
        IF ( PARLL.AND.IONOD(MXMIP+1).EQ.-9999 )                          40.30
     &     IONOD(MXMIP+1) = NINT(OUTR(1))                                 40.30
...
!
 ...
 65       FORMAT('XCMIN        ,XCMAX  ,YCMIN     ,YCMAX    ,XPMIN',
     ...
     &           YC(IP).GE.-0.01 .AND. YC(IP).LE.REAL(MYC-1)+0.01 ) THEN  40.31
               IF ( KGRPNT(NINT(XC(IP))+1,NINT(YC(IP))+1).GT.1 ) THEN     40.31
...
!  4. Argument variables
!
!     IONOD : array indicating in which subdomain and at what time        40.31
!             output points are located                                   40.31
...
!
 ...
 121    FORMAT (' SWOEXD, type:', 4I3)
        CALL SWIPOL (COMPDA(1,JDP1), OVEXCV(4), XC, YC, MIP,
     &               VOQ(1,VOQR(4)) ,KGRPNT, COMPDA(1,JDP1))
        print*,'kkkkk',COMPDA(KGRPNT(10,10),JDP1)
      ENDIF
!
!       current velocity
!
...
!         subdomain                                                       40.31
      ...
      IF ( PARLL ) THEN                                                   40.31
         ...
         IF ( LMXL ) IXE = MXC                                            40.31
         IYB = 1+IHALOY                                                   40.31
         IF ( LMYF ) IYB = 1                                              40.31
         IYE = MYC-IHALOY                                                 40.31
         IF ( LMYL ) IYE = MYC                                            40.31
         DO IP = 1, MIP                                                   40.31
...
!     IONOD   input  array indicating in which subdomain and at what      40.31
!                    time output points are located                       40.31
