...
!
!  7. Common Blocks used (updated 30.72)
!
!     COMMON / CBOUP  / *** ???
...
!     COMMON / SWANWL / *** variables for project h3268                   32.01
...
!  8. Subroutines used
!
!     SPCVAR: Reading the variable spectral boundary conditions from file 32.01
...
!           **The number of computational steps is calculated
     ...
     &           'DT is not a fraction of the computational period')
          TIMCO = TINIC
        ENDIF
!   redistribute time variables: TINIC, TFINC, MTC for master program -fyshi
...
!
        ...
        IF (KEYWIS ('REFL')) THEN                                         40.09
          ...
          DUM = REF0+TRCF                                                 40.14
          IF ((DUM < 0.) .OR. (DUM > 1.)) THEN                            40.14
            CALL MSGERR(3,'C_tr + C_ref > 1 or C_tr + C_ref < 0! ')       40.14
          ENDIF                                                           40.14
...
!
     ...
     &                 'At least, FLOW and FHIGH or MSC must be given!')  40.31
        ...
        IF (IVAL.NE.999) THEN                                             40.31
           GAMMA = EXP(ALOG(FRHIG/FRLOW)/REAL(MSS))                       40.31
     ...
     &                    ' Number of meshes in sigma-space: MSC = ',MSS  40.31
...
!     ------------------------------------------------------------------
!
!     LIM     sets values for coefficients used in conjunction with the limiters
!
! ============================================================
!
...
!     ------------------------------------------------------------------
!
!     NUM     data numerical scheme
!
! ============================================================
!
! NUMeric ACCUR  (  [drel] [dhoval] [dtoval] [npnts]                  &   40.03
!
...
!
!
!           | -> SIGIMpl [css] [prec] [eps1] [eps2] [outp] [niter] |
!          (<                                                      >) &
...
!
!
!           ( SETUP [prec] [eps2] [outp] [niter]                   )      30.82
!
!
! ============================================================
!
...
!
!     ------------------------------------------------------------------
!     SETUP    setup due to waves is computed and taken into account
!
!     SETUP   [supcor]                                                    30.82
!
      IF (KEYWIS ('SETUP')) THEN                                          32.02
        IF (KSPHER.NE.0) CALL MSGERR (2,
     &  'setup is not computed correctly with spherical coordinates')     40.13
        LSETUP = 1                                                        32.02
...
! ------------------------------------------------------------------
!
!   SET: Set value of physical parameters and error counters
!
! =============================================================
!
...
!
      ...
      ENDIF
!     ------------------------------------------------------------------
!     QUANTITY    set parameters for output quantities                    40.03
!
...
!
      IF (KEYWIS ('TRI')) THEN
        ...
        IF (KEYWIS('EXP')) THEN
          ITRIAD = ITRIAD + 1
!         limiter activated only for explicit triads
          PNUMS(20) = 0.1                                                 40.00
        ELSE
          CALL IGNORE ('IMP')
        ENDIF
        ...
        CALL INREAL ('URSLIM', PTRIAD(5), 'STA', 0.01)                    40.23 40.13
...
!     --- Create copy of parts of KGRBGL for each subdomain -> KGRBND     40.31
      ...
      IF(.NOT.ALLOCATED(AC2)) ALLOCATE(AC2(MDC,MSC,MCGRD))                40.31
      AC2 = 0.                                                            40.31
      LOGCOM(6) = .TRUE.
!
...
!
!         neighbouring points on straight line (i.e. infact 1D)
        IF ((WNP.EQ.2) .AND.(
...
!     --- append node number to FILENM in case of parallel computing      40.31
 ...
 103    FORMAT (I6, T41, A)                                               40.00
     ...
     &         EQREAL(YCGRID(IX,IY), EXCFLD(9)) ) THEN
            WRITE (NREF, FMT=CRFORM) EXCFLD(8), EXCFLD(9)
          ELSE
