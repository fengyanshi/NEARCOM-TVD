...
!
     ...
     &                   ' with 1D-computation')                          32.02
        ...
        ELSE                                                              32.02
          ...
          IF (MIP.GT.MXMIP) MXMIP = MIP                                   40.31
...
!
          ...
          NREOQ = NREOQ + 1                                               40.31
          IF (NREOQ.GT.MXOURQ) MXOURQ = NREOQ                             40.31 40.30
...
!             option disabled                                             40.13
          ...
          ELSE
            ...
            RTYPE = 'BLKP'
            IF ( INDEX( FILENM, '.MAT' ).NE.0 .OR.                        40.30
     &           INDEX (FILENM, '.mat' ).NE.0 ) THEN                      40.30
...
!          <  DSPR/HSIGN/DIR/PDIR/TDIR/TM01/RTM01/RTP/TM02/FSPR/DEPTH/VEL/  &
!             FRCOEFF/WIND/DISSIP/QB/TRANSP/FORCE/UBOT/URMS/WLEN/STEEPNESS/ &
!             DHSIGN/DRTM01/LEAK/XP/YP/DIST  [unit]                         &
...
!
!       Ocean Pack command reading routines
!       BC_POINTS : boundary points interpolation
!
...
!             If b.c. type is NEST
!             Then calculate data on grid points calling BC_POINTS subroutine,
...
!           in other cases coordinates are ignored
            IF (BCTYPE .EQ. 'NEST') THEN
              ...
              YP2 = YP
!
!             call subroutine BC_POINTS to interpolate the boundaries
!             points to the grid points of the SWAN computational grid
!
              NBGRPT_PREV = NBGRPT                                        40.05
              CALL BC_POINTS (  LBGP, XCGRID, YCGRID,                     40.31 40.05
...
!      interpolate them to the grid pionts of the SWAN computational grid using
!      subroutine BC_POINTS
!
!  4. Argument variables
!
...
!     number of direction NANG,  number of boundaries  pionts NBOUNC,
...
!       of the boundary piont to local Cartesian coordinates
!
          IF (IBOUNC.EQ.1) THEN
...
!
        ...
        ENDIF
!
!       call subroutine BC_POINTS to interpolate the boundaries points to the grid
!       points of the SWAN computational grid
!
        NBGRPT_PREV = NBGRPT
        CALL BC_POINTS (  LBGP, XCGRID, YCGRID,                           40.31
...
!***********************************************************************
!
      SUBROUTINE BC_POINTS ( LBGP, XCGRID, YCGRID,                        40.31
...
!  2. Purpose
!
!     interpolation of points grid  to the swan-computational  grid
!
!  3. Method
!
!     calculate interpolation coefficients betweens the  nesting  boundary grid and
!     the swan-computational  grid
!
!  4. Argument variables
!
!     NBOUNC:     max number of boundaries points
!     BOUND_COUN: counter show  of the existing  boundary point
!
!
...
!     DISMAX    largest distance
!     SUMWGT    sum of interpolation weights
!     W2        is relative length of projection on line
...
!               else(defaule case)  DONALL is FALSE
!                   boundary is close, it do interpolation between the last
!                   and the first point
...
!           loop over boundary of comp. grid, select points between
!           (XP1,YP1) and (XP2,YP2)
!           KGRBND grid addresses on boundary pionts, NGRBND number of grid points
