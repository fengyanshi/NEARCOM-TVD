...
! channel
          ...
          if(dep(i,j).lt.dep_inlet*cos(pi/(2.0*600.0)*x(i,j)))
     &       dep(i,j)=dep_inlet*cos(pi/(2.0*600.0)*x(i,j))
          endif
! basin
...
! for swan, make a minimum depth to get equal grid decomposition
       do j=1,n
        ...
        open(1,file='dep_shoal_inlet_Ushape.txt')
         do j=1,n
...
! for swan
         ...
         open(1,file='dep_swan_shoal_inlet_Ushape.txt')
         do j=1,n
